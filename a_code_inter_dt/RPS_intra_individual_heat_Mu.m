...
RPS_intra_individual_heat_Mu(100,0,10000,1.5,0.5,1,1,60,5.2,1.0,0.5)
sur=load('sur1.csv');
plot(sur(:,1),'bo');hold on;plot(sur(:,2),'ro');hold on;plot(sur(:,3),'go');hold on;plot(sur(:,4),'ko');legend('A','B','C','empty')

function RPS_intra_individual_heat_Mu(Lsize,pre,ite,re1,re2,re3,selection_rate,mobility,intra1,intra2,intra3)
...
%         figure(2); heatmap(Lattice,'Colormap',summer);
    ...
    dlmwrite('sur1.csv',sur,'delimiter',',','-append');
end
